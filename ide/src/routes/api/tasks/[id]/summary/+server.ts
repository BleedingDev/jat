 ...
 * 3. Calling the IDE helper LLM (Codex-native-first) to generate a structured summary
 *
...
Output ONLY the JSON object.`;
		...
		// Try to generate summary using IDE LLM provider (Codex-native-first)
		const schemaPath = getLlmSchemaPath('task-summary-schema.json') ?? undefined;
		...
		try {
			...
			const raw = parseJsonResponse<Record<string, unknown>>(llmResponse.result);
			const summaryLines = Array.isArray(raw.summary) ? raw.summary.filter((s): s is string => typeof s === 'string').slice(0, 10) : [];
			const keyChanges = Array.isArray(raw.keyChanges) ? raw.keyChanges.filter((s): s is string => typeof s === 'string').slice(0, 50) : [];
			const outcome = raw.outcome === 'completed' || raw.outcome === 'incomplete' || raw.outcome === 'blocked'
				? raw.outcome
				: (hasCompletion ? 'completed' : (taskData.status === 'blocked' ? 'blocked' : 'incomplete'));
			...
			const summary: TaskSummary = {
				...
				agent,
				keyChanges,
				quality: (raw.quality as TaskSummary['quality']) || undefined,
				suggestedTasks: (raw.suggestedTasks as TaskSummary['suggestedTasks']) || [],
				crossAgentIntel: (raw.crossAgentIntel as TaskSummary['crossAgentIntel']) || undefined,
				riskLevel: (raw.riskLevel as TaskSummary['riskLevel']) || undefined,
				breakingChanges: (raw.breakingChanges as TaskSummary['breakingChanges']) || undefined,
				generatedAt: new Date().toISOString()
			};
