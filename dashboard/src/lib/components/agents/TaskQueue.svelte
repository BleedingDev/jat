			...
			{#each filteredTasks as task (task.id)}
				...
				>
					<div class="card-body p-3 relative">
						<!-- Task ID badge on left with copy button -->
						<div class="absolute -top-2 left-2 flex items-center gap-0.5 group/taskid">
							<span class="badge badge-sm badge-ghost text-xs text-base-content/50 font-mono">
								{task.id}
							</span>
							<button
								class="opacity-0 group-hover/taskid:opacity-100 transition-opacity btn btn-xs btn-ghost btn-square p-0 h-4 w-4 min-h-0"
								title="Copy task ID"
