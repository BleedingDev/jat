...
<!-- Industrial Agent Count Badge -->
<div class="flex items-center gap-2" title={activeTooltip}>
	<!-- Active Count (green badge with pulsing dot) - Industrial -->
	<div class="flex items-center gap-1.5">
		{#if activeCount > 0}
			<!-- Pulsing live indicator - Industrial green -->
		...
		>
			...
			{#if !compact}
				<span style="color: oklch(0.50 0.02 250);">active</span>
			{/if}
		</span>
	</div>
	...
	>
		...
		{#if !compact}
			<span style="color: oklch(0.45 0.02 250);">total</span>
		{/if}
	</span>
</div>
