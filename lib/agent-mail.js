 ...
 * @param {string | undefined} [projectPath] - Optional project path to filter by
 * @returns {Reservation[]} - Array of reservation objects with agent and project info
 */
export function getReservations(agentName = undefined, projectPath = undefined) {
  try {
    ...
    if (projectPath !== undefined) {
      // Accept either a full path match OR a last-segment match.
      // This supports both absolute paths (e.g., /home/user/code/jat)
      // and short project keys used by the IDE/tests (e.g., "jat").
      query += ' AND (p.human_key = ? OR p.human_key LIKE ?)';
    ...
    const stmt = db.prepare(query);
    /** @type {Reservation[]} */
    const reservations = /** @type {Reservation[]} */ (stmt.all(...params));
